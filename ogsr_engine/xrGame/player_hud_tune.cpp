...
void attachable_hud_item::tune(const Ivector& values)
{
    ...
    if (g_bHudAdjustMode == HUD_ADDON_ATTACH_POS || g_bHudAdjustMode == HUD_ADDON_ATTACH_ROT || g_bHudAdjustMode == WORLD_ADDON_ATTACH_POS ||
        g_bHudAdjustMode == WORLD_ADDON_ATTACH_ROT || g_bHudAdjustMode == HUD_ADDON_ATTACH_SCALE || g_bHudAdjustMode == WORLD_ADDON_ATTACH_SCALE)
    {
        if (g_bHudAdjustMode == HUD_ADDON_ATTACH_POS || g_bHudAdjustMode == WORLD_ADDON_ATTACH_POS || g_bHudAdjustMode == HUD_ADDON_ATTACH_SCALE ||
            g_bHudAdjustMode == WORLD_ADDON_ATTACH_SCALE)
        {
            if (values.x)
                diff.x = (values.x > 0) ? g_bHudAdjustDeltaPos : -g_bHudAdjustDeltaPos;
            if (values.y)
...
void player_hud::DumpParamsToLog()
{
    ...
    else
    {
        ...
        else if (g_bHudAdjustMode == HUD_ADDON_ATTACH_POS || g_bHudAdjustMode == HUD_ADDON_ATTACH_ROT || g_bHudAdjustMode == WORLD_ADDON_ATTACH_POS ||
                 g_bHudAdjustMode == WORLD_ADDON_ATTACH_ROT || g_bHudAdjustMode == HUD_ADDON_ATTACH_SCALE || g_bHudAdjustMode == WORLD_ADDON_ATTACH_SCALE)
        {
            if (auto Wpn = smart_cast<CWeapon*>(m_attached_items[g_bHudAdjustItemIdx]->m_parent_hud_item))
            {
                for (int i = 0; i < eMaxAddon; ++i)
                {
                    if ((g_bHudAdjustMode == HUD_ADDON_ATTACH_POS || g_bHudAdjustMode == HUD_ADDON_ATTACH_ROT || g_bHudAdjustMode == HUD_ADDON_ATTACH_SCALE) &&
                        Wpn->hud_attach_visual[i])
                    {
                        Log("####################################");
                        Msg("[%s]", hud_sect);
                    ...
                    if ((g_bHudAdjustMode == WORLD_ADDON_ATTACH_POS || g_bHudAdjustMode == WORLD_ADDON_ATTACH_ROT || g_bHudAdjustMode == WORLD_ADDON_ATTACH_SCALE) &&
                        Wpn->world_attach_visual[i])
                    {
                        Log("####################################");
...
void hud_draw_adjust_mode()
{
    ...
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT))
        _text =
            ...
            "SHIFT+1-laser_point|2-flashlight_point|3-custom_ui_pos|4-custom_ui_rot|5-hud_addon_attach_pos|6-hud_addon_attach_rot|7-world_addon_attach_pos|8-world_addon_attach_"
            "rot|9-hud_addon_attach_scale|0-world_addon_attach_scale";
