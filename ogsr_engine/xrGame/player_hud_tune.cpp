...
static constexpr std::array<std::tuple<int, const char*>, _HUD_ADJUST_MODES_COUNT_> ADJUST_MODES_DB{{
    ...
    {DIK_7, "adjusting HUD ADDON ATTACH SCALE"},
}};
...
void attachable_hud_item::tune(const Ivector& values)
{
    ...
    {
        if (g_bHudAdjustMode == HUD_ADDON_ATTACH_POS || g_bHudAdjustMode == HUD_ADDON_ATTACH_SCALE)
        {
            if (values.x)
                diff.x = (values.x > 0) ? g_bHudAdjustDeltaPos : -g_bHudAdjustDeltaPos;
            if (values.y)
        ...
        else if (g_bHudAdjustMode == HUD_ADDON_ATTACH_ROT)
        {
            if (values.x)
                diff.x = (values.x > 0) ? g_bHudAdjustDeltaRot : -g_bHudAdjustDeltaRot;
            if (values.y)
...
void player_hud::DumpParamsToLog()
{
    ...
    else
    {
        ...
        else if (g_bHudAdjustMode == HUD_ADDON_ATTACH_POS || g_bHudAdjustMode == HUD_ADDON_ATTACH_ROT || g_bHudAdjustMode == HUD_ADDON_ATTACH_SCALE)
        {
...
void hud_draw_adjust_mode()
{
    ...
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT))
        _text =
            ...
            "SHIFT+1-laser_point|2-flashlight_point|3-custom_ui_pos|4-custom_ui_rot|5-hud_addon_attach_pos|6-hud_addon_attach_rot|7-hud_addon_attach_scale";
