...
static void draw_rect(const Frect& r, const u32 color, const shared_str& name)
{
    ...
    if (psHUD_Flags.test(HUD_SHOW_WND_RECT_NAMES) && name.size())
    {
...
void CUIWindow::Draw()
{
    ...
    if (psHUD_Flags.test(HUD_SHOW_WND_RECT_ALL))
    {
        Frect r;
        GetAbsoluteRect(r);
...
void CUIWindow::UpdateFocus(bool focus_lost)
{
    ...
    if (cursor_on_window && psHUD_Flags.test(HUD_SHOW_WND_RECT))
    {
        Frect r;
        GetAbsoluteRect(r);
        add_rect_to_draw(r, m_windowName);
...
bool CUIWindow::OnMouse(float x, float y, EUIMessages mouse_action)
{
    ...
    case WINDOW_LBUTTON_UP: OnMouseUp(MOUSE_1); break;
    case WINDOW_RBUTTON_UP: OnMouseUp(MOUSE_2); break;
