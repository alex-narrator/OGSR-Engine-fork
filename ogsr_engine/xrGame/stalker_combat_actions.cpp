...
void CStalkerActionRetreatFromEnemy::execute()
{
    ...
    object().movement().set_path_type(MovementManager::ePathTypeLevelPath);
    object().movement().set_detail_path_type(DetailPathManager::eDetailPathTypeSmooth);
    object().movement().set_mental_state(/*eMentalStatePanic*/ eMentalStateDanger); //Panic animation looks ridiculous, danger set is better
    object().movement().set_body_state(eBodyStateStand);
...
void CStalkerCombatActionThrowGrenade::initialize()
{
    ...
    const CInventoryItem* grenade = object().inventory().m_slots[GRENADE_SLOT].m_pIItem;
    VERIFY(grenade);
