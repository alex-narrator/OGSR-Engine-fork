...
u16 map_add_user_spot_level_id(u8 level_id, Fvector position, LPCSTR spot_type, LPCSTR text)
{
...
u16 map_add_user_spot_level_name(LPCSTR level_name, Fvector position, LPCSTR spot_type, LPCSTR text)
{
    CMapLocation* ml = Level().MapManager().AddUserLocation(spot_type, level_name, position);
    if (xr_strlen(text))
        ml->SetHint(text);
...
void CLevel::script_register(lua_State* L)
{
    module(L)[(class_<CEnvDescriptor>("CEnvDescriptor")
        ...
        module(L, "level")[(
            ...
            def("map_add_user_spot", &map_add_user_spot_level_id), def("map_add_user_spot", &map_add_user_spot_level_name), def("map_get_spot_hint", &map_get_spot_hint),
