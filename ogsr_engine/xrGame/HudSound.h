...
struct HUD_SOUND
{
    ...
    static void LoadSound(CInifile* ini, LPCSTR section, LPCSTR line, ref_sound& hud_snd, int type = sg_SourceType, float* volume = nullptr, float* delay = nullptr,
                          float* freq = NULL);

    static void LoadSound(LPCSTR section, LPCSTR line, HUD_SOUND& hud_snd, int type = sg_SourceType);
    static void LoadSound(CInifile* ini, LPCSTR section, LPCSTR line, HUD_SOUND& hud_snd, int type = sg_SourceType);
...
public:
    ...
    void PlaySound(LPCSTR alias, const Fvector& position, const CObject* parent, bool hud_mode, bool looped = false, bool overlap = false);

    void StopSound(LPCSTR alias);

    void LoadSound(LPCSTR section, LPCSTR line, LPCSTR alias, int type = sg_SourceType);
    void LoadSound(CInifile* ini, LPCSTR section, LPCSTR line, LPCSTR alias, int type = sg_SourceType);
...
public:
    ...
    HUD_SOUND* FindSoundItem(LPCSTR alias, bool b_assert);
    void PlaySound(LPCSTR alias, const Fvector& position, const CObject* parent, bool hud_mode, bool looped = false, bool overlap = false);
    void StopSound(LPCSTR alias);
    void StopAllSounds();
    void LoadSound(LPCSTR section, LPCSTR line, LPCSTR alias, int type = sg_SourceType);
    void LoadSound(CInifile* ini, LPCSTR section, LPCSTR line, LPCSTR alias, int type = sg_SourceType);
    void SetPosition(LPCSTR alias, const Fvector& pos);
    void UpdateAllSoundsPositions(const Fvector& pos);
};
//-Alundaio